...
```
```{r AIC selection subs_day3}
X_scale <- data_interactions[,-1] %>% scale() %>% data.frame()
y <- subs_day3 %>% scale()
train <-which(allocation == "train")
...
model_data <- data.frame(y = subs_day3, x = data_interactions[,coefficients_3])

for (i in 2:length(coefficients_3)) {
  plot(y = subs_day3, x = model_data[,i])
