...
```{r PCA for betta}
X <- data.frame(data_interactions[,-1], data_square) %>% scale(center = TRUE, scale = TRUE) %>% as.matrix()
  
  pca <- prcomp(X)
  betta <- abs(pca$x[1,])
  reg_pca <- lm(formula = subs_day3 ~ ., data = data_interactions[,-1][,which(betta > 0.15)])
  newpred <- forecast(object = reg_pca, newdata = data_interactions[,-1][,which(betta > 0.15)])$mean
  sqrt(mse(subs_day3, newpred))
